...
#define EXE_NAME "vbam"
  ...
  if(f != NULL)
  {
	fprintf(stderr,"loading vbam.cfg\n");
    return f;
  }
#endif // ! _WIN32
...
void sdlReadPreferences()
{
 //  FILE *f = sdlFindFile("vbam.cfg");
  ...
  chmod("/accounts/1000/shared/misc/vbampb", 0777);

  FILE *f = fopen("/accounts/1000/shared/misc/vbampb/vbam.cfg","r");

  if(f == NULL) {
    fprintf(stderr, "/accounts/1000/shared/misc/vbampb/vbam.cfg not found... \n");
    f = fopen("vbam_default.cfg","r");
    if(f == NULL)
    {
      fprintf(stderr,"./vbam_default.cfg failed to load ...\n");
    }
    return;
  } else
    fprintf(stderr, "Reading configuration file ../shared/misc/vbampb/vbam.cfg \n");
...
void sdlInitVideo() {
 ...
 fprintf(stderr,"sdlInitVideo: %d x %d\n", screenWidth, screenHeight);
  ...
  fprintf(stderr,"SDL video mode set ...");
  SDL_ShowCursor(0);
...
//#endif
   ...
   //  IMAGE_TYPE type = utilFindType(szFile);
    ...
    if(type == IMAGE_GB) {
      ...
      if(!failed) {
        ...
        fprintf(stderr,"CPUInit: '%s' %d", biosFileName, useBios);
 ...
 // int flags = SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER;
  ...
  fprintf(stderr,"initialize filter ...\n");
  ...
  fprintf(stderr,"Color depth: %d  update color maps ...\n", systemColorDepth);
